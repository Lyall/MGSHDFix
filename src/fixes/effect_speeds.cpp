...
// Focus actors can run fast. They read their script messages in Act, so they run every frame;
// only the focus rack is held to the PS2's 30 Hz.
#define MGS2_FOCUS_PACE_HOOKS(X) \
    X(Act_FarFocus, "48 89 5C 24 ?? 57 48 83 EC ?? 48 8B D9 48 8D 54 24 ?? 8B 49 58 E8 ?? ?? ?? ?? 33 FF 44 8B C8 85 C0 75 ?? 39 7B 60 0F 84 ?? ?? ?? ?? 39 3D ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? 8B 43 58 81 4B 10 00 00 01 00 89 43 14 48 8B 5C 24 ?? 48 83 C4 ?? 5F C3 4C", "MGS 2: Effect Speed Fix : user\\takabe\\effect1\\f_focus.c -> Act()") \
    X(Act_NearFocus, "48 89 5C 24 ?? 57 48 83 EC ?? 48 8B D9 48 8D 54 24 ?? 8B 49 58 E8 ?? ?? ?? ?? 33 FF 44 8B C8 85 C0 75 ?? 39 7B 60 0F 84 ?? ?? ?? ?? 39 3D ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? 8B 43 58 81 4B 10 00 00 01 00 89 43 14 48 8B 5C 24 ?? 48 83 C4 ?? 5F C3 0F", "MGS 2: Effect Speed Fix : user\\takabe\\effect1\\n_focus.c -> Act()")
...
namespace
{
    ...
    int64_t FocusActAtPs2Pace(SafetyHookInline& hook, int64_t work)
    {
        constexpr ptrdiff_t kTargetNear = 0x68, kTargetFar = 0x6C, kNear = 0x70, kFar = 0x74, kInterp = 0x78;
        const auto field = [work](ptrdiff_t off) { return reinterpret_cast<uint8_t*>(work) + off; };
        const bool hold = SkipFrame();
        const int interpBefore = *reinterpret_cast<int*>(field(kInterp));
        const float nearBefore = *reinterpret_cast<float*>(field(kNear));
        const float farBefore = *reinterpret_cast<float*>(field(kFar));
        const float targetNearBefore = *reinterpret_cast<float*>(field(kTargetNear));
        const float targetFarBefore = *reinterpret_cast<float*>(field(kTargetFar));

        const int64_t result = hook.call<int64_t>(work);

        if (hold && interpBefore > 0 &&
            *reinterpret_cast<float*>(field(kTargetNear)) == targetNearBefore &&
            *reinterpret_cast<float*>(field(kTargetFar)) == targetFarBefore)
        {
            *reinterpret_cast<int*>(field(kInterp)) = interpBefore;
            *reinterpret_cast<float*>(field(kNear)) = nearBefore;
            *reinterpret_cast<float*>(field(kFar)) = farBefore;
        }
        return result;
    }

#define DEFINE_MGS2_FOCUS_PACE_HOOK(name, pattern, label) \
    SafetyHookInline name##_hook {}; \
    int64_t __fastcall name##_Hook(int64_t work) { return FocusActAtPs2Pace(name##_hook, work); }

    MGS2_FOCUS_PACE_HOOKS(DEFINE_MGS2_FOCUS_PACE_HOOK)

#undef DEFINE_MGS2_FOCUS_PACE_HOOK
...
#define INSTALL_MGS2_FRAMESKIP_HOOK(name, pattern, label) \
    ...
    MGS2_FOCUS_PACE_HOOKS(INSTALL_MGS2_FRAMESKIP_HOOK)
